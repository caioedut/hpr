...
describe('Collection', () => {
    ...
    it('should collapse a collection of collections', () => {
        const result = collect([[1, 2], [3, 4], [5, 6]]).collapse();
        expect(result.all()).toEqual([1, 2, 3, 4, 5, 6]);
    });
    ...
    it('should group items by a key', () => {
        const result = collect([{id: 1, name: 'A'}, {id: 2, name: 'B'}, {id: 1, name: 'C'}])
            .groupBy('id');
        expect(result.all()).toEqual({1: [{id: 1, name: 'A'}, {id: 1, name: 'C'}], 2: [{id: 2, name: 'B'}]});
    });
    ...
    it('should pluck a value from the collection', () => {
        const result = collect([{name: 'A', age: 20}, {name: 'B', age: 30}]).pluck('name');
    ...
    it('should cross join two collections', () => {
        ...
        expect(result.all()).toEqual([[1, 3], [1, 4], [2, 3], [2, 4]]);
    });
